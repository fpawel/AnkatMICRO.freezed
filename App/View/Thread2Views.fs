...
module Delay = 
    ...
    let initialize =         
        ...
        Ankat.PartyWorks.Delay.onStart.Value <- fun what get'time -> 
        ...
        Ankat.PartyWorks.Delay.onStop.Value <- fun () -> 
            MainWindow.form.PerformThreadSafeAction <| fun () ->
                panelDelay.Visible <- false
                panelPerformingInfo.Visible <- true
            

        Ankat.PartyWorks.Delay.onUpdate.Value <- fun start'time get'time -> 
