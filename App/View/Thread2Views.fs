...
module Delay = 
    ...
    let initialize =         
        ...
        Ankat.PartyWorks.Delay.onStart.Value <- fun what get'time -> p.PerformThreadSafeAction <| fun () ->
            text <- what
            progressBar.Value <- 0                
        ...
        Ankat.PartyWorks.Delay.onStop.Value <- fun () -> p.PerformThreadSafeAction <| fun () ->
            performing'bar.DoUpdate <| fun () ->                    
                performing'bar.BottomControl <- None

        Ankat.PartyWorks.Delay.onUpdate.Value <- upd
        fun () -> ()
...
let initialize = 
    ...
    Ankat.PartyWorks.ModalMessage.onShow.Value <- fun title level text ->            
    ...
    Ankat.PartyWorks.ModalMessage.onClose.Value <- fun () ->            
        form.PerformThreadSafeAction <| fun () -> 
            modal'message'bar.Visible <- false

    Ankat.PartyWorks.ModalMessage.getIsVivisble.Value <- fun () ->
